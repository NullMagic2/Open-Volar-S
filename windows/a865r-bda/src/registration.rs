...
pub fn register(user: bool) -> Result<()> {
    // Wine 10's device enumerator opens HKCR, but registrations in HKCU may
    // not be visible there. HKLM belongs to the user's Wine prefix and does
    // not require Linux root privileges. Native Windows retains user scope.
    let wine = a865r::transport::wine_bridge::is_wine();
    let root = if user && !wine {
        HKEY_CURRENT_USER
    } else {
        HKEY_LOCAL_MACHINE
    };
    ...
    for (clsid, cat, label) in [
        ...
        // Standard System Device Enumerator monikers are needed by VLC and other
        // BDA clients even when the underlying USB transport is WinUSB.
        set(root, &instance, "CLSID", &clsid)?;
        set(root, &instance, "FriendlyName", &format!("Open Volar S A865R ISDB-T {label}"))?;

    }
    if user && wine {
        unregister_at(HKEY_CURRENT_USER)?;
    }
    crate::trace(format!(
        "registered scope={} path={path}",
        if wine { "wine-prefix" } else if user { "user" } else { "system" }
    ));
    Ok(())
}
pub fn unregister(user: bool) -> Result<()> {
    ...
    unregister_at(root)?;
    if user && wine {
        // Remove registrations left by the earlier per-user Wine installer.
        unregister_at(HKEY_CURRENT_USER)?;
    }
    Ok(())
}
fn unregister_at(root: HKEY) -> Result<()> {
    for (clsid, cat) in [
