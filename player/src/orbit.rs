...
impl Skin {
    ...
    pub unsafe fn settings_widget(&self,dc:HDC,r:RECT,tabs:HWND,current:usize,dpi:u32) {
        ...
        let tab_rect=|i:usize,selected:bool| {
            ...
            RECT{left:if selected && i==0 {body.left+3}else if selected {left.max(body.left+u(9))}else{left},
                ...
                top:r.top+u(if selected {48}else{55}),bottom:body.top+u(4)}
        };
    ...
    pub(super) unsafe fn button_face(&self,a:&App,dc:HDC,draw:&DRAWITEMSTRUCT,r:RECT)->ButtonPaint {
        ...
        let texture = cache.entry(key).or_insert_with(|| {
            ...
            for y in 0..h {
                for x in 0..w {
                    let (sx, sy) = if material == Material::Metal {
                        let scale = source.w as f32 / (425. * key.3 as f32 / 96.);
                        (
                            ((x as f32 * scale) as i32) % source.w,
                            ((y as f32 * scale) as i32) % source.h,
                        )
                    } else {
                        (x * source.w / w, y * source.h / h)
                    };
                    let from = ((sy * source.w + sx) * 4) as usize;
                    let at = ((y * w + x) * 4) as usize;
                    let lighting = if material == Material::Metal {
                        18. * (1. - x as f32 / w as f32) - 15. * y as f32 / h as f32
                    } else {
                        0.
                    };
                    let glass = if material == Material::Glass {
                        0.09 * (-((y as f32 / h as f32 - 0.03) / 0.24).powi(2)).exp()
                    } else {
                        0.
                    };
                    for c in 0..3 {
                        let base = source.bgra[from + c] as f32 + lighting;
                        let warm = [176., 218., 244.][c];
                        pixels[at + c] = (base * (1. - glass) + warm * glass).clamp(0., 255.) as u8;
                    }
                    pixels[at + 3] = 255;
                }
            }
    ...
    pub unsafe fn button(&self, a: &App, dc: HDC, draw: &DRAWITEMSTRUCT) {
        ...
        let font_size = (h as f32 * 0.31).round().max(11.) as i32;
        let f = font(dc, font_size);
        let text=i18n::text(&text);
        let s = wide(&text);
        let mut size = SIZE::default();
        let _ = GetTextExtentPoint32W(dc, &s[..s.len() - 1], &mut size);
        let icon_size = (h as f32 * 0.36).round() as i32;
        let gap = if icon.is_empty() || text.is_empty() { 0 } else { (h / 7).max(4) };
