...
pub fn record(
    ...
    match crate::recording_finalize::finish(&capture_path,&path,program,&ffmpeg,&folder.join("recording-validation"),&control) {
        Ok(report)=>{playback["recording"]=json!(path);playback["recording_validation"]=report;}
        Err(e)=>{playback["error"]=json!(e);playback["broadcast_capture"]=json!(capture_path);}
    }
    playback
}
