...
bool TextRendererDirectWrite::SetFontFamily(const std::vector<std::string>& font_family) {
    ...
    if (font_family_ != font_family) {
        font_family_ = font_family;
        // The face and DirectWrite format must change together. A stream can
        // receive its language metadata after its first rendered caption.
        main_faceinfo_.reset();
        fallback_faceinfo_.reset();
        main_face_index_ = 0;
        main_text_format_.Reset();
        fallback_text_format_.Reset();
        main_text_format_pixel_height_ = 0;
        fallback_text_format_pixel_height_ = 0;
    }
    return true;
}
