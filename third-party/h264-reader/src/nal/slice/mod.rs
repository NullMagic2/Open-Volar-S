...
impl PredWeightTable {
    fn read<R: BitRead>(
        ...
        let mut luma_weights_l1 = Vec::new();
        let mut chroma_weights_l1 = Vec::new();
        if slice_type.family == SliceFamily::B {
            let count = match num_ref_active {
                Some(NumRefIdxActive::B { num_ref_idx_l1_active_minus1, .. }) => *num_ref_idx_l1_active_minus1,
                _ => pps.num_ref_idx_l1_default_active_minus1,
            };
            for _ in 0..=count {
                luma_weights_l1.push(if r.read_bool("luma_weight_l1_flag")? {
                    Some(PredWeight {
