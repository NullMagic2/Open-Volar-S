...
pub struct VulkanDecoder<'a> {
    initialized_session: Option<vk::VideoSessionKHR>,
    pub(crate) video_session_resources: Option<VideoSessionResources<'a>>,
    pub(crate) tracker: DecoderTracker,
    pub(crate) reference_id_to_dpb_slot_index: FxHashMap<ReferenceId, usize>,
    pub(crate) decoding_device: Arc<DecodingDevice>,
...
impl<'a, 'b> DecodeSubmission<'a, 'b> {
    pub(crate) fn download_output(self) -> Result<DecodeResult<RawFrameData>, VulkanDecoderError> {
    ...
    #[cfg(feature = "wgpu")]
    pub(crate) fn output_to_wgpu_texture(self) -> Result<DecodeResult<wgpu::Texture>, VulkanDecoderError> {
        // Diagnostic only: distinguish video-engine failure from texture interop.
        if std::env::var_os("A865R_VK_VALIDATE").is_some() {
            self.decoder.tracker.wait_for(self.semaphore_wait_value, 5_000_000_000)
                .map_err(|e| VulkanDecoderError::InvalidInputData(format!("Video decode completion before texture transfer: {e}")))?;
            if let Some(pool) = &self.decode_query_pool {
                pool.check_results_blocking()?;
            }
        }
...
impl<'a> VulkanDecoder<'a> {
    pub(crate) fn ensure_broadcast_session(&mut self) -> Result<(), VulkanDecoderError> {
        if let Some(session) = self.video_session_resources.as_mut() {
            session.ensure_session(&self.decoding_device,
                self.tracker.command_buffer_pools.decode.begin_buffer()?, &mut self.tracker)?;
        }
        Ok(())
    }
    ...
    pub(crate) fn process_sps(&mut self, sps: &SeqParameterSet) -> Result<(), VulkanDecoderError> {
        match self.video_session_resources.as_mut() {
            Some(session) => session.process_sps(sps.clone(), self.usage_info, &self.decoding_device)?,
            None => {
    ...
    pub(crate) fn do_decode<'b>(
        ...
        let mut reference_slots = video_session_resources.decoding_images.reference_slot_info();
        for slot in &mut reference_slots {
            let active = decode_information.reference_list_l0.as_ref().into_iter().flatten()
                .chain(decode_information.reference_list_l1.as_ref().into_iter().flatten())
                .any(|r| self.reference_id_to_dpb_slot_index.get(&r.id).is_some_and(|i|*i as i32==slot.slot_index));
            if !active {slot.slot_index=-1;}
        }
        ...
        // Every new session must be initialized, independently of the H.264
        // picture type (VUID-vkCmdDecodeVideoKHR-None-07011).
        if is_idr || self.initialized_session != Some(video_session_resources.video_session.session) {
        ...
        let new_reference_slot_index = if decode_information.second_field {
            *self.reference_id_to_dpb_slot_index.get(&reference_id)
        ...
        let std_picture_info = decode_information.native_picture.unwrap_or_else(|| {
            let header = decode_information.header.as_ref().expect("parsed slice header");
        ...
        if crate::wrappers::validation_failed() {
            return Err(VulkanDecoderError::InvalidInputData("Vulkan validation rejected decode commands before submission".into()));
        }
        ...
        self.initialized_session = Some(video_session_resources.video_session.session);
    ...
    fn output_to_wgpu_texture(
        ...
        let mut queue_indices = vec![self.decoding_device.queues.transfer.family_index as u32,
            self.decoding_device.queues.wgpu.family_index as u32];
        queue_indices.sort_unstable();queue_indices.dedup();
        ...
        let create_info = vk::ImageCreateInfo::default()
            ...
            .sharing_mode(if queue_indices.len()>1 {vk::SharingMode::CONCURRENT}else{vk::SharingMode::EXCLUSIVE})
