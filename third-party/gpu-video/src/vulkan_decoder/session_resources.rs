...
pub(crate) struct VideoSessionResources<'a> {
...
fn active_capacity(sps:&SeqParameterSet,device:&DecodingDevice)->u32 {
    let multiplier=if matches!(sps.frame_mbs_flags,FrameMbsFlags::Frames){1}else{2};
    (sps.max_num_ref_frames*multiplier).min(device.profile_capabilities.video_capabilities.max_active_reference_pictures)
}
...
impl<'a> VideoSessionResources<'a> {
    pub(crate) fn new_from_sps(
        ...
        let max_active_references = active_capacity(&sps, decoding_device);
    ...
    pub(crate) fn process_sps(
        ...
        let new_session_params = SessionParams {
            ...
            max_active_references: active_capacity(&sps, decoding_device),
