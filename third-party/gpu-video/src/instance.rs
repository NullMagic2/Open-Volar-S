...
impl VulkanInstance {
    ...
    fn new_from_entry(entry: Arc<Entry>) -> Result<Arc<Self>, VulkanInitError> {
        ...
        if cfg!(feature = "vk-validation") || std::env::var_os("A865R_VK_VALIDATE").is_some() {
        ...
        let debug_messenger = if cfg!(debug_assertions) || std::env::var_os("A865R_VK_VALIDATE").is_some() {
