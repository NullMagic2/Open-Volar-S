...
static VALIDATION_FAILED: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
pub(crate) fn validation_failed() -> bool {
    VALIDATION_FAILED.load(std::sync::atomic::Ordering::Acquire)
}
...
unsafe extern "system" fn debug_messenger_callback(
    ...
    // Keep the existing known Naga SPIR-V layout diagnostic filtered; it is independent of video decoding.
    if callback_data.message_id_number == 0xb210f7c2u32 as i32 { return vk::FALSE; }
    if let Some(path)=std::env::var_os("A865R_VK_VALIDATE") {
        use std::io::Write;
        if let Ok(mut file)=std::fs::OpenOptions::new().create(true).append(true).open(path) {
            let message=unsafe {callback_data.message_as_c_str().unwrap_or(c"")};
            let _=writeln!(file,"{:?}: {}",message_severity,message.to_string_lossy());
        }
        if message_severity.contains(vk::DebugUtilsMessageSeverityFlagsEXT::ERROR) {
            VALIDATION_FAILED.store(true,std::sync::atomic::Ordering::Release);
            // Diagnostic mode refuses calls rejected by validation.
            return vk::TRUE;
        }
    }
