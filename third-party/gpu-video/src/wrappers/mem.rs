...
// The whole aligned range is passed to vkCmdDecodeVideoKHR. Reused buffer
// padding must not contain the preceding picture's compressed bytes.
fn write_decode_bitstream(destination: &mut [u8], data: &[u8]) {
    destination[..data.len()].copy_from_slice(data);
    destination[data.len()..].fill(0);
}
#[cfg(test)] mod decode_padding_tests {
    #[test] fn reused_alignment_tail_is_zeroed_without_changing_payload() {
        let mut destination=[0x65;256];
        super::write_decode_bitstream(&mut destination,&[0,0,1,0x61,0xab]);
        assert_eq!(&destination[..5],&[0,0,1,0x61,0xab]);
        assert!(destination[5..].iter().all(|b|*b==0));
        super::write_decode_bitstream(&mut destination,&[0,0,1,0x65]);
        assert!(destination[4..].iter().all(|b|*b==0));
    }
}
...
impl DecodeInputBuffer {
    ...
    pub(crate) fn upload_data(
        ...
        if data.len() as u64 > size || size > usize::MAX as u64 {
            return Err(VulkanDecoderError::InvalidInputData("Invalid aligned decode buffer size".into()));
        }
        ...
        unsafe {
            ...
            let slice = std::slice::from_raw_parts_mut(mem.cast(), size as usize);
            write_decode_bitstream(slice, data);
