...
impl VulkanDevice {
    /// Retire a failed driver device without blocking playback recovery.
    pub fn mark_lost(&self) {
        self.device.lost.store(true,std::sync::atomic::Ordering::Release);
    }
    ...
    pub(crate) fn broadcast_decoding_device(self:&Arc<Self>, profile:&vk::VideoProfileInfoKHR) -> Result<DecodingDevice,VulkanDecoderError> {
        let profile_capabilities=caps::NativeDecodeProfileCapabilities::<H264Codec>::query(
            &self.device._instance,self._physical_device,profile
        ).map_err(|e|VulkanDecoderError::InvalidInputData(format!("Unsupported broadcast video profile: {e}")))?;
        Ok(DecodingDevice {vulkan_device:self.clone(),h264_decode_queues:self.queues.h264_decode.clone()
            .ok_or(VulkanDecoderError::VulkanDecoderUnsupported)?,profile_capabilities})
    }
