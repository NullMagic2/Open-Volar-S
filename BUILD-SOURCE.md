...
These instructions describe the **0.8.0-alpha.43** source package. Commands are
...
- For the complete installer: the Rust **i686-pc-windows-msvc** target,
  **Inno Setup 6**, and the matching x64 Microsoft C++ redistributable DLLs
  expected by `installer/build.ps1`.
...
`Open-Volar-S-Setup-0.8.0-alpha.43-x64.exe`, next to the workspace directory.
...
third-party notices. It excludes compiled applications/DLLs, Cargo build output,
...
SHA-256 hash. It does not hash itself. The GitHub documentation package keeps
the application version at **0.8.0-alpha.43**; no runtime source changes are
implied by the documentation refresh.
...
original project material; its scope and full text are in [LICENSE.md](LICENSE.md).
The root [LICENSE](LICENSE) and all third-party notices remain in place.
