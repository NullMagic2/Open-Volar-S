...
impl WinUsbTransport {
    ...
    fn enumerate_bulk_pipes(&mut self) -> Result<()> {
        ...
        let ok =
            unsafe { WinUsb_QueryInterfaceSettings(self.interface_handle, 0, &mut descriptor) };
        if ok == 0 {
        ...
        for index in 0..descriptor.b_num_endpoints {
            ...
            let ok = unsafe { WinUsb_QueryPipe(self.interface_handle, 0, index, &mut pipe) };
            if ok == 0 {
    ...
    fn close_device_handles(&mut self) {
        ...
        unsafe {
            if !self.interface_handle.is_null() {
                let _ = WinUsb_Free(self.interface_handle);
    ...
    fn find_device() -> Result<FoundDevice> {
        ...
        for index in 0u32.. {
            let mut interface_data = SpDeviceInterfaceData {
                cb_size: size_of::<SpDeviceInterfaceData>() as u32,
                interface_class_guid: A865R_INTERFACE_GUID,
                flags: 0,
                reserved: 0,
            };
        ...
        Err(Error::Transport(
            "no A865R WinUSB interface was found. Install the development WinUSB binding first"
                .to_string(),
        ))
    }
}
...
impl Transport for WinUsbTransport {
    ...
    fn open(&mut self, _vendor_id: u16, _product_id: u16) -> Result<()> {
        ...
        // SAFETY: handle is a valid device handle returned by CreateFileW and output pointer is writable.
        let ok = unsafe { WinUsb_Initialize(handle, &mut interface_handle) };
        if ok == 0 {
            ...
            return Err(Self::winusb_initialize_error(code));
        }
    ...
    fn set_command_pipes(&mut self, output_pipe: u8, input_pipe: u8) -> Result<()> {
        ...
        unsafe {
            let _ = WinUsb_AbortPipe(self.interface_handle, output_pipe);
            let _ = WinUsb_AbortPipe(self.interface_handle, input_pipe);
            let _ = WinUsb_FlushPipe(self.interface_handle, input_pipe);
        }
    ...
    fn exchange(
        ...
        let ok = unsafe {
            WinUsb_WritePipe(
                self.interface_handle,
                self.command_out_pipe,
                request_copy.as_mut_ptr(),
                request_len,
                &mut written,
                null_mut(),
            )
    ...
    fn description(&self) -> String {
        if self.device_path.is_empty() {
            ...
            format!("WinUSB: {}", self.device_path)
        }
    }
}
