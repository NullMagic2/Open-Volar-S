...
use interaction::{channel_number,channel_match};
...
impl App {
    ...
    fn sort_saved_channels(&mut self) {
        let order=interaction::channel_order(&self.services,self.channels.len());
        self.channel_index=order.iter().position(|&i|i==self.channel_index).unwrap_or(0);
        self.channels=order.iter().map(|&i|self.channels[i]).collect();
        self.services=order.iter().map(|&i|self.services.get(i).cloned().unwrap_or(Value::Null)).collect();
    }
    unsafe fn merge_channels(&mut self, found: &[Value]) {
        ...
        updated.sort_by_key(interaction::channel_sort_key);
    ...
    unsafe fn layout_deck(&self) {
        ...
        for (id,x,y,w) in orbit::translated_buttons(self.deck,r) {
            let y=y-30.;
            ...
            })
        {
            if self.services.get(self.channel_index) != Some(&state["service"]) {
                ...
                if let Some(s) = self.services.get_mut(self.channel_index) {
                    let number=s["channel_number"].clone();
                    *s = state["service"].clone();
                    if s["channel_number"].is_null(){s["channel_number"]=number;}
                }
                self.sort_saved_channels();
                self.update_channels();
                self.save();
            }
        }
    ...
    unsafe fn command(&mut self, hwnd: HWND, id: u16, notification: u16) {
        match id {
            ...
            i18n::LANGUAGE if notification==CBN_SELCHANGE as u16=>{
                ...
                self.layout_deck();self.layout();guide::language_changed();
            }
...
unsafe fn run() -> windows::core::Result<()> {
    ...
    let scale = wine_ui::dpi(GetDpiForSystem()) as i32;
    ...
    let video = CreateWindowExW(
        WINDOW_EX_STYLE(0),
        class,
        w!("Live TV! — 0.9.5 • Vulkan preview"),
    ...
    if GetMonitorInfoW(MonitorFromWindow(deck, MONITOR_DEFAULTTONEAREST), &mut screen).as_bool() {
        ...
        let [stream, panel] = window_placement::centered_pair(
            ...
            wine_ui::dpi(GetDpiForWindow(deck)),
        );
