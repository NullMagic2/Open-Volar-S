...
impl Backend {
    ...
    pub fn label(self) -> &'static str { if wine(){return "Linux playback (Wine)";} match self {Self::Vulkan=>"Vulkan GPU",Self::Microsoft=>"Microsoft processing"} }
    pub fn choices(microsoft: bool) -> Vec<Self> {
        if wine(){return vec![Self::Vulkan];}
...
fn wine()->bool {
    #[cfg(windows)] {a865r::transport::wine_bridge::is_wine()}
...
pub fn capabilities()->Capabilities {
    if wine(){return Capabilities{vulkan:true,dx12:false,dx11:false};}
